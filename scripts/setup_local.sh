...
if str(cfg.get("llm_provider", "")).strip().lower() == "local_ollama":
    cfg.pop("llm_provider", None)

cfg["openrouter_base_url"] = "https://openrouter.ai/api/v1"
cfg["openrouter_api_key"] = str(cfg.get("openrouter_api_key", "") or "")
cfg["openrouter_model"] = str(cfg.get("openrouter_model", "") or "")
...
print(f"[setup] openrouter_base_url={cfg.get('openrouter_base_url')}")
print(f"[setup] openrouter_model={cfg.get('openrouter_model')}")
